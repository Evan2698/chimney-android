...
if [ ! -f "$TARGET/armeabi-v7a/libss-local-ex.so" ] || [ ! -f "$TARGET/arm64-v8a/libss-local-ex.so" ] ||
   [ ! -f "$TARGET/x86/libss-local-ex.so" ]; then
    ...
    echo "Cross compile overture for arm"
    if [ ! -f "$TARGET/armeabi-v7a/libss-local-ex.so" ]; then
        try env CGO_ENABLED=1 CC=$ANDROID_ARM_CC GOOS=android GOARCH=arm GOARM=7 go build -tags "DRCLO" -ldflags="-s -w"
        try $ANDROID_ARM_STRIP local
        try mv local $TARGET/armeabi-v7a/libss-local-ex.so
    fi

    echo "Cross compile overture for arm64"
    if [ ! -f "$TARGET/arm64-v8a/libss-local-ex.so" ]; then
        try env CGO_ENABLED=1 CC=$ANDROID_ARM64_CC GOOS=android GOARCH=arm64 go build -tags "DRCLO" -ldflags="-s -w"
        try $ANDROID_ARM64_STRIP local
        try mv local $TARGET/arm64-v8a/libss-local-ex.so
    fi

    echo "Cross compile overture for x86"
    if [ ! -f "$TARGET/x86/libss-local-ex.so" ]; then
        try env CGO_ENABLED=1 CC=$ANDROID_X86_CC GOOS=android GOARCH=386 go build -tags "DRCLO" -ldflags="-s -w"
        try $ANDROID_X86_STRIP local
        try mv local $TARGET/x86/libss-local-ex.so
    fi
