...
object BaseService {
    ...
    class Data internal constructor(private val service: Interface) {
        ...
        internal fun buildShadowsocksConfig(): File {
            ...
            val file = File((if (UserManagerCompat.isUserUnlocked(app)) app.deviceContext.filesDir else @TargetApi(24) {
    ...
    interface Interface {
        ...
        fun startNativeProcesses() {
           ...
           /* val cmd = buildAdditionalArguments(arrayListOf(
            ...
            var cfgpath = data.buildShadowsocksConfig().absolutePath

            if ( cfgpath != null && cfgpath.length > 0) {

                var cmd = ArrayList<String>()
                cmd.add(File((this as Context).applicationInfo.nativeLibraryDir, Executable.SS_LOCAL_EX).absolutePath)
                cmd.add("-c")
                cmd.add(CONFIG_FILE)
                ...
                Log.e(tag, "create config file failed!!!!!" )
            }
        }
